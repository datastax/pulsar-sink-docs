...
The {product} allows for mapping a single topic to multiple tables for a single connector instance.
...
In the {company} keyspace `stocks_keyspace`, create three different tables that optimized with different schemas.
...
Configure the {product-short} and add all the map specifications:
