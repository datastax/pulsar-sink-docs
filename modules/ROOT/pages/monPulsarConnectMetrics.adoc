...
You can view {product} metrics in raw Prometheus format at the {pulsar} metrics endpoint: `http://**PULSAR_INSTANCE**:8080/metrics`.

You can also https://prometheus.io/docs/prometheus/latest/getting_started/[configure a Prometheus instance to monitor your {pulsar-short} instance].
...
The sink name refers to a configured and running {product-short} sink, and metric class is the general class of the metric, such as `Count` or `SnapShotSize`.
...
Integer count for various {product-short} metrics.
...
failedRecordCount::
Running total of the number of failed records encountered by the {product-short} instance.
+
.Example
[source,console]
----
...
The {product-short}'s lifetime average number of records processed.

cql-requests::
Average number of CQL requests processed since the {product-short} was created.
+
.Example
[source,console]
----
...
recordCount::
Average number of records processed since the {product-short} was created.
+
.Example
[source,console]
----
...
failedRecordCount::
Average number of failed records processed since the {product-short} was created.
+
.Example
[source,console]
----
...
failedRecordsWithUnknownTopic::
Average number of failed records with unknown topics processed since the {product-short} was created.
+
.Example
[source,console]
----
