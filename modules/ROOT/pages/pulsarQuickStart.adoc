...
It covers installation and configuration of {dse-short}, {pulsar}, and the {product}.
...
[source,cql]
----
CREATE KEYSPACE IF NOT EXISTS pulsar_qs
WITH replication = {
...
[source,cql]
----
CREATE TABLE pulsar_qs.pulsar_kv (
	key text PRIMARY KEY,
	content text
);
----

== Install {pulsar} and the {product-short}
...
+
Use a topic name in the format _keyspace_._table_ (for example, `pulsar_qs.pulsar_kv`), not a generic name like `example_topic`.
+
.qs.yml
[source,yaml]
----
...
. In a new terminal, check that the {product-short} is running:
+
[source,bash]
----
...
----
+
Make sure the response includes the {product-short}:
+
[source,json]
----
[
  {
    ...
    "description":"A DataStax Pulsar Sink to load records from Pulsar topics to Apache Cassandra or DataStax Enterprise(DSE)\n",
...
+
Use a sink task class name that matches your topic data format.
The connector supports two options:
+
* `com.datastax.oss.sink.pulsar.RecordCassandraSinkTask`: Use this class name by default.
It works with all supported logical data types.
The data topic typically exports messages in key-value Avro format.
* `com.datastax.oss.sink.pulsar.StringCassandraSinkTask`: Use when the data topic carries JSON or loosely structured data.
This class name might not work for some data types such as decimal, tuples, date, and timeuuid.
+
[source,bash]
----
bin/pulsar-admin sinks create \
	...
	--tenant public \
	--namespace default \
	--inputs "persistent://public/default/pulsar_qs.pulsar_kv"
----
+
The topic name must use the format _keyspace_._table_ (for example, `pulsar_qs.pulsar_kv`).
The topic name and mapping are set in the {product-short} configuration YAML file.

. Send some messages to your new sink:
+
[source,bash]
----
bin/pulsar-client produce -k "Message 1" -m "Content 1" persistent://public/default/pulsar_qs.pulsar_kv
bin/pulsar-client produce -k "Message 2" -m "Content 2" persistent://public/default/pulsar_qs.pulsar_kv
bin/pulsar-client produce -k "Message 3" -m "Content 3" persistent://public/default/pulsar_qs.pulsar_kv
----
...
[source,cql]
----
SELECT * FROM pulsar_qs.pulsar_kv;
----
+
...
* xref:ROOT:cfgPulsarDisplayingTopic.adoc[Print messages] to determine the data structure of the topic messages.

* Create xref:ROOT:cfgPulsarMapKeyPair.adoc[key-value pair mappings], xref:ROOT:cfgPulsarMapJson.adoc[JSON mappings], and xref:ROOT:cfgPulsarMapAvroMessages.adoc[Avro mappings].

* Try advanced and specialized mapping functionality, such as xref:ROOT:cfgPulsarMapUdt.adoc[mapping UDTs].

* Use one connector instance to xref:ROOT:cfgPulsarMapMultipleTables.adoc[ingest a single topic into multiple tables] or xref:ROOT:cfgPulsarMultTopicToTables.adoc[ingest multiple topics to different tables].