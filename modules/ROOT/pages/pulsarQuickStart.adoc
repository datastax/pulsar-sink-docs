...
+
Use a sink task class name that matches your topic data format. The connector supports two options:
+
* `com.datastax.oss.sink.pulsar.RecordCassandraSinkTask` — Use this class name by default. It works with all supported logical data types. The data topic typically exports messages in key-value Avro format.
* `com.datastax.oss.sink.pulsar.StringCassandraSinkTask` — Use when the data topic carries JSON or loosely structured data. This class name might not work for some data types such as decimal, tuples, date, and timeuuid.
+
[source,bash]
----
...
----
+
The topic name must use the format _keyspace_._table_ (for example, `pulsar_qs.pulsar_kv`). The topic name and mapping are set in the {product-short} configuration YAML file.
