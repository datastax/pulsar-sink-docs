...
The system requirements for {product} depends on the workload and network capacity.
The factors include characteristics of the {pulsar-short} topic and the cluster data models and volume.
{company} recommends testing with realistic data flows before committing to an instance type for the connector.

Database::
The {product} supports the following databases:
+
* {company} {astra-db}
...
{pulsar-short}::
The {product} requires {pulsar} 2.7.0 or later.
+
The connector supports the following data structures in {pulsar-short} topics:
+
...
CPU::
{product} is bound by the amount of CPU available on the host.
The connector holds all the records pulled from {pulsar-short} topics in memory, along with the cluster metadata and prepared statements.
+
...
Network::
{product} needs adequate network capacity for the payload.
...
+
The {product} framework automatically rebalances the load when workers are added by reallocating the tasks among the workers.
...
== Start {pulsar-short} with the connector
...
Explore the rest of the documentation to learn more about configuring and using the {product}.