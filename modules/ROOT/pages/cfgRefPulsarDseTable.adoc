...
The connector writes messages from your {pulsar-short} topics to tables in your database using the mapping and topic properties set in your connector's configuration YAML file.
...
[#topics]
topics::
A comma separated list of all topics to which the {product} subscribes.
...
+
If `true`, the {product} treats null values as unset fields.
...
However, the connector inserts the record without the TTL because the {pulsar-short} connector is a streaming system.
--
======
+
If TTL is set (either statically or dynamically), the same TTL limit is applied to _all_ rows in the mapped table.
The {product} appends `AND TTL <seconds>` to all `INSERT` statements for the table.
+
With either static or dynamic TTL, you can optionally set `ttlTimeUnit` to specify the time unit for the `ttl` value.
The default time unit is `SECONDS`.
If you specify a different time unit, such as `HOURS`, the {product} automatically converts the value to `SECONDS` for compatibility.
...
Use the `__timestamp` property when your {pulsar-short} records have an explicit timestamp field that you want to use as the writetime for the database record produced by the connector.
+
...
If you specify a different time unit, such as `SECONDS`, the {product} automatically converts the value to `MICROSECONDS` for compatibility.
