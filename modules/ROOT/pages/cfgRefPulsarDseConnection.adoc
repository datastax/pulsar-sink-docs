...
+
When connecting to {astra-db} with `<<ssl-encrypted-connections,cloud.secureConnectBundle>>`, leave this property unset.
+
Default: `""`
...
If the cluster has client encryption enabled, configure the <<ssl-encrypted-connections,SSL keys and certificates in the {pulsar-short} connector>>.
...
+
--
To connect to {astra-db}, provide the application token credentials in the <<authentication-parameters,authentication parameters>>, and use the {scb} to establish an <<ssl-encrypted-connections,SSL-encrypted connection>>.
--
======
...
* **`{dse-short}`**: Internal or LDAP authentication
...
+
You can provide the literal path to your {scb-short} or encode the zip file using the standard base64 tool:
+
[source,bash]
----
base64 -i scb.zip
