...
In your {product} configuration file, you can pass some connection parameters directly to the embedded {cass} Java driver by using the `datastax-java-driver` prefix.
For example:
...
The following list maps {product} parameters to their functionally equivalent Java driver parameters:
...
You can authenticate the {product} session using internal, LDAP, Kerberos, or application token authentication, depending on the methods supported by your cluster.
...
The connector supports {dse-short} role management with internal and LDAP schemes.
...
If xref:6.9@dse:managing:configure/configure-dse-yaml.adoc#authorizationOptions[authorization is enabled], this login role must have a minimum of `modify` privileges on the tables that receive data from the {product}.
+
For {astra-db}, enter the literal string `token` or provide the `clientId` from an {astra} application token.
The application token's role must have read/write privileges on the tables that receive data from the {product}.
