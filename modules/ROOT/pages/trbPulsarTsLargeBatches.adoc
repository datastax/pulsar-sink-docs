...
The connector requires that you set the `loadBalancing.localDc` for DSE clusters.

Check the datacenter name in the cluster by running `xref:dse:managing:tools/nodetool/status.adoc[nodetool status]` and verify that xref:ROOT:cfgRefPulsarDseConnection.adoc#loadBalancing[loadBalancing.localDc] exactly matches because datacenter names are case sensitive.
...
You can decrease the batch size by lowering the number of records collected in each batch in xref:cfgRefPulsarDseConnection.adoc#maxNumberOfRecordsInBatch[maxNumberOfRecordsInBatch].
