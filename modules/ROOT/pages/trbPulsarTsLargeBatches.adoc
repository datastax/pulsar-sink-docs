...
:navtitle: Troubleshoot the {product-short}
...
This page describes some common issues that you might encounter when using the {product} and how to resolve them.

== Load balancing datacenter is not specified

The following error message occurs when the load balancing datacenter isn't set in the {product-short}'s configuration file:
...
The {product-short} requires that you set the `loadBalancing.localDc` for {dse-short} clusters when `contactPoints` isn't set to `localhost`.
...
For {astra-db}, the datacenter is set by the {scb}.
Make sure your connector configuration file includes a valid path to the database's {scb-short} zip file.

== Writes fail because of mutation size

The {pulsar-short} connector collects records to write to the database in CQL `xref:dse-6.9@cql:reference:cql-commands/batch.adoc[BATCH]` commands.
Data insertions and deletions in the records are known as mutations.
The {product-short} is implemented to use single-partition batches that are submitted as one mutation operation.

For a given batch, if the total size of the mutation exceeds the maximum allowed by {dse-short} in `xref:dse:managing:configure/configure-cassandra-yaml.adoc#max_mutation_size_in_kb[max_mutation_size_in_kb]` the batch is rejected and the error message is written to the `system.log`, by default in `var/log/cassandra`.
...
You can decrease the batch size by lowering the number of records collected in each batch in `xref:ROOT:cfgRefPulsarDseConnection.adoc[maxNumberOfRecordsInBatch]`.
...
You can also increase the {dse-short} database batch threshold using `xref:dse:managing:configure/configure-cassandra-yaml.adoc#guardrailsYaml[batch_size_warn_threshold_in_kb]`.
...
. Change the {product-short}'s `verbose` option to `true` in your sink configuration file:
+
[source,yaml]
----
configs:
...
When the data from a {pulsar-short} field isn't compatible with the data type of the mapped column in the target table, the data conversion fails and a message is logged in the {pulsar-short} Connect log.
For example:
...
Ensure that the {pulsar-short} fields are mapped to the correct database columns.
The {product-short} can not automatically convert varying types.

If they do not match, consider changing your table schema to accommodate the {pulsar-short} field types.

When adding or changing columns in a database table, ensure that the schema is fully propagated before continuing.
With {dse-short}, use `xref:dse:managing:tools/nodetool/describe-ring.adoc[nodetool describering]` to show the schema version.