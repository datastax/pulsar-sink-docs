...
The DataStax Apache Kafka Connector collects records to write to the DataStax Enterrpise database in CQL link:/en/dse/6.7/cql/cql/cql_reference/cql_commands/cqlBatch.html[BATCH] commands.
...
You can decrease the batch size by lowering the number of records collected in each batch (link:../configuration_reference/kafkaDseConnection.md#maxNumberOfRecordsInBatch[maxNumberOfRecordsInBatch]).
The default is 32.
