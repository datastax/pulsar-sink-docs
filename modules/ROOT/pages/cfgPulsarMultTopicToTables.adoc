= Map multiple topics to multiple tables

The {product} allows for mapping multiple topics to multiple tables in a single connector instance.

NOTE: Most {pulsar-reg} systems store all messages in the same format and {product-short} workers only support a single converter class for key and value.
