...
The DataStax Apache Pulsar(TM) connector can process JSON, Avro, and complex types directly into xref:dse-6.9@cql:develop:user-defined-type-overview.adoc[user-defined type] columns.
...
If your mapping includes maps or UDTs, you can xref:ROOT:cfgPulsarCqlQuery.adoc[use CQL queries in your connector configuration] to selectively update maps and UDTs based on the existence of values in the Pulsar fields.
...
It uses the CQL `UPDATE` command to only update the fields present in the Pulsar record.
