...
The DataStax Apache Pulsar(TM) Connector processes JSON, Avro, and complex types directly into a xref:dse-6.9@cql:develop:user-defined-type-overview.adoc[user-defined type] column in the database. The Pulsar message embedded field names and the UDT field names must match.
