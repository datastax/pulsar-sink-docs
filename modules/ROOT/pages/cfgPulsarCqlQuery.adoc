...
In your connector configuration, you can provide a CQL query to run when a new record arrives on the {pulsar-short} topic.

[IMPORTANT]
====
If you define a query in the mapping, the query has priority, and it is used instead of the {product-short}'s automatically generated action.

This is an advanced feature for specific use cases.
Typically, you can use the standard {pulsar-short} mapping without a query.
====
