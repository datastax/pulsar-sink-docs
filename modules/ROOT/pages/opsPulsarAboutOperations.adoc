= Operate and maintain the DataStax Apache Pulsar(TM) connector
:navtitle: Pulsar connector administration
...
To operate and maintain the DataStax Apache Pulsar(TM) connector, use the https://pulsar.apache.org/docs/4.1.x/admin-api-tools/[Apache Pulsar administration tools], which include the pulsar-admin CLI and the Pulsar admin API.
...
Use the `pulsar-admin sinks update` command to update the configuration of an active Pulsar sink.
...
* `**OPTIONS**`: https://pulsar.apache.org/docs/next/io-cli/#update-1[Pulsar sink configuration options] that you want to update, such as `--sink-config-file`, `--parallelism`, or `--timeout-ms`.
